...
mockMasterServer = http.createServer( (req, resp) ->
  switch req.url.toLowerCase()
    when '/', '/html'
      ...
      resp.writeHead 200
      resp.end """<h2>Some awesome content for you to check out 
    ...
    when '/js'
      ...
      resp.end '(function() { var t= "t"; })();'
    when '/cached'
      resp.writeHead(200, {
        'Content-Type': 'text/html',
        'Cache-Control': 'max-age=3'
      })
      resp.end """This content was retrieved from master 
                at: #{Date().toString()}
